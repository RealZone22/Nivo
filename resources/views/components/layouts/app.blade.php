...
<div x-data="{ sidebarIsOpen: false }" class="relative flex w-full flex-col md:flex-row">
    ...
    <div class="h-svh w-full overflow-y-auto bg-surface dark:bg-surface-dark">
        ...
        @persist('player')
        <nav
            x-data="{ playing:false, duration:0, current:0, volume:1, cover:'{{ asset('img/default-cover.png') }}', init() { const a=this.$refs.audio; a.addEventListener('loadedmetadata', ()=>{ this.duration = a.duration }); a.addEventListener('timeupdate', ()=>{ this.current = a.currentTime }); a.addEventListener('ended', ()=>{ this.playing = false }); a.volume = this.volume; }, toggle() { const a=this.$refs.audio; if(a.paused){ a.play(); this.playing = true } else { a.pause(); this.playing = false } }, seek(e){ const rect = e.target.getBoundingClientRect(); const pct = (e.clientX - rect.left)/rect.width; this.$refs.audio.currentTime = pct * this.duration }, setVolume(e){ this.volume = e.target.value; this.$refs.audio.volume = this.volume } }"
            x-init="init()"
